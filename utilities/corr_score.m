...
while size(df_map{i},2)<maxROI 
df_map{i}=[df_map{i} nan(size(df_map{i},1),1)]; %replace missing last ROI with NaN
end
end
...
for i = sessions
rate_map_tuned{i}=rate_map{i}(:,tuned_ROI);
df_map_tuned{i}=df_map{i}(:,tuned_ROI);
end
...
[PVmatrix_df{i}{ii}] = corr(df_map{i}',df_map{ii}', 'rows', 'complete');

matrixnb{i}{ii}=[i ii];
    %tuned ROI
[PVmatrix_tuned{i}{ii}] = corr(rate_map_tuned{i}',rate_map_tuned{ii}', 'rows', 'complete');
[PVmatrix_df_tuned{i}{ii}] = corr(df_map_tuned{i}',df_map_tuned{ii}', 'rows', 'complete');
...
[PVcorr_df{i}{ii}(:,iii)] = corr(df_map{i}(iii,:)',df_map{ii}(iii,:)', 'rows', 'complete');
[PVcorr_df_tuned{i}{ii}(:,iii)] = corr(df_map_tuned{i}(iii,:)',df_map_tuned{ii}(iii,:)', 'rows', 'complete');

PVcorr_mean{i}{ii}= nanmean(PVcorr{i}{ii});
PVcorr_tuned_mean{i}{ii}=nanmean(PVcorr_tuned{i}{ii});
PVcorr_df_mean{i}{ii}= nanmean(PVcorr_df{i}{ii});
PVcorr_df_tuned_mean{i}{ii}=nanmean(PVcorr_df_tuned{i}{ii});
end
end
end
...
[TCcorr_df{i}{ii}(iii)] = corr(df_map{i}(:,iii),df_map{ii}(:,iii), 'rows', 'complete');
TCcorrind{i}{ii}=[i ii];
end
end
end
...
TCcorr_df_tuned{i}{ii}=TCcorr_df{i}{ii}(:,tuned_ROI);
TCcorr_mean{i}{ii}= nanmean(TCcorr{i}{ii});
TCcorr_tuned_mean{i}{ii}= nanmean(TCcorr_tuned{i}{ii});
TCcorr_df_mean{i}{ii}= nanmean(TCcorr{i}{ii});
TCcorr_df_tuned_mean{i}{ii}= nanmean(TCcorr_tuned{i}{ii});
end
end 
%Make structure
...
colormap(rate_cmap);
title({'All ROI normalized rate ', [Spatial_correlation.options.name{plot_corr{i}(1)}, ' VS ', Spatial_correlation.options.name{plot_corr{i}(end)} ]})
end
figure; 
for i=1:size(plot_corr,2)
    subplot(1,size(plot_corr,2),i)
imagesc(PVmatrix_df{plot_corr{i}(1)}{plot_corr{i}(end)});
colormap(rate_cmap);
title({'All ROI normalized mean dF/F s-1', [Spatial_correlation.options.name{plot_corr{i}(1)}, ' VS ', Spatial_correlation.options.name{plot_corr{i}(end)} ]})
end
%Matrix tuned
figure; 
for i=1:size(plot_corr,2)
...
colormap(rate_cmap);
title({'Tuned ROI normalized rate', [Spatial_correlation.options.name{plot_corr{i}(1)}, ' VS ', Spatial_correlation.options.name{plot_corr{i}(end)} ]})
end
figure; 
for i=1:size(plot_corr,2)
    subplot(1,size(plot_corr,2),i)
imagesc(PVmatrix_df_tuned{plot_corr{i}(1)}{plot_corr{i}(end)});
colormap(rate_cmap);
title({'Tuned ROI normalized mean dF/F s-1', [Spatial_correlation.options.name{plot_corr{i}(1)}, ' VS ', Spatial_correlation.options.name{plot_corr{i}(end)} ]})
end

for i=1:size(plot_corr,2)
PV_corr_test(:,i)=PVcorr{plot_corr{i}(1)}{plot_corr{i}(end)};
PV_corr_tuned_test(:,i)=PVcorr_tuned{plot_corr{i}(1)}{plot_corr{i}(end)};
PV_corr_df_test(:,i)=PVcorr_df{plot_corr{i}(1)}{plot_corr{i}(end)};
PV_corr_df_tuned_test(:,i)=PVcorr_df_tuned{plot_corr{i}(1)}{plot_corr{i}(end)};


PVmatrix_test=PVmatrix{plot_corr{i}(1)}{plot_corr{i}(end)};
PVmatrix_tuned_test=PVmatrix_tuned{plot_corr{i}(1)}{plot_corr{i}(end)};
PVmatrix_df_test=PVmatrix_df{plot_corr{i}(1)}{plot_corr{i}(end)};
PVmatrix_df_tuned_test=PVmatrix_df_tuned{plot_corr{i}(1)}{plot_corr{i}(end)};

TC_corr_test(:,i)=TCcorr{plot_corr{i}(1)}{plot_corr{i}(end)};
TC_corr_tuned_test(:,i)=TCcorr_tuned{plot_corr{i}(1)}{plot_corr{i}(end)};
TC_corr_df_test(:,i)=TCcorr_df{plot_corr{i}(1)}{plot_corr{i}(end)};
TC_corr_df_tuned_test(:,i)=TCcorr_df_tuned{plot_corr{i}(1)}{plot_corr{i}(end)};
end 
...
Spatial_correlation.PV_correlation.AllROI.PV_matrix_dF=PVmatrix_df_test;
Spatial_correlation.PV_correlation.AllROI.PVcorr_dF=PV_corr_df_test;
...
Spatial_correlation.PV_correlation.TunedROI.PV_matrix_dF=PVmatrix_df_tuned_test;
Spatial_correlation.PV_correlation.TunedROI.PVcorr_dF_tuned=PV_corr_df_tuned_test;
...
Spatial_correlation.TC_correlation.AllROI.TCcorr_dF=TC_corr_df_test;
Spatial_correlation.TC_correlation.TunedROI.TCcorr_dF=TC_corr_df_tuned_test;
